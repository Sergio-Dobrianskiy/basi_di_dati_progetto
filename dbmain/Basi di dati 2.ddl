...
-- * Generation date: Sat Aug 24 23:18:52 2024 
-- * LUN file: D:\uni\basi_di_dati_progetto\dbmain\Basi di dati.lun 
...
create database ProgettoLogico;
use ProgettoLogico;
...
create table CARTE_CREDITO (
     ...
     cognome_associato char(1) not null,
     nome_associato char(1) not null,
...
create table ENTI (
     id_ente int not null,
     descrizione_ente char(1) not null,
     saldo int not null,
...
create table LISTINO_ABBONAMENTI (
     ...
     descrizione_abbonamento char(1) not null,
...
create table RUOLI (
     ...
     descrizione_ruolo char(1) not null,
...
create table SERVIZI (
     ...
     descrizione_servizio char(1) not null,
...
create table SERVIZI_ACQUISTATI (
     ...
     prezzo_pagato char(1) not null,
     media_recensioni int not null,
     num_carta_credito int not null,
     id_city_card int not null,
     id_servizio int not null,
     constraint IDSERVIZI_ACQUISTATI primary key (id_acquisto));
...
alter table CARTE_CREDITO add constraint FKR
     foreign key (id_user)
...
alter table CHECKS add constraint FKR_1
     foreign key (id_stato)
     references STATI_CHECK (id_stato);

alter table CHECKS add constraint FKR
     foreign key (id_city_card)
     references CITY_CARD (id_city_card);

alter table COLLABORAZIONI add constraint FKR
     foreign key (id_user)
     references USERS (id_user);

alter table COLLABORAZIONI add constraint FKR_1
     foreign key (id_ente)
     references ENTI (id_ente);

alter table ENTI add constraint FKR
     foreign key (id_user)
     references USERS (id_user);

alter table EVENTI add constraint FKR
     foreign key (id_ente)
     references ENTI (id_ente);

alter table EVENTI add constraint FKR_1_FK
     foreign key (id_periodo)
     references PERIODI (id_periodo);

alter table LISTINO_ABBONAMENTI add constraint FKR
     foreign key (id_sconto)
     references SCONTI (id_sconto);

alter table PARTECIPAZIONI add constraint FKR
     foreign key (id_evento)
     references EVENTI (id_evento);

alter table PARTECIPAZIONI add constraint FKR_1
     foreign key (id_city_card)
     references CITY_CARD (id_city_card);
...
alter table RECENSIONI add constraint FKR
     foreign key (id_servizio)
     references SERVIZI (id_servizio);

alter table RECENSIONI add constraint FKR_1
     foreign key (id_user)
     references USERS (id_user);
...
alter table SERVIZI_ACQUISTATI add constraint FKR_1
     foreign key (num_carta_credito)
     references CARTE_CREDITO (num_carta_credito);

alter table SERVIZI_ACQUISTATI add constraint FKR_2
     foreign key (id_city_card)
     references CITY_CARD (id_city_card);

alter table SERVIZI_ACQUISTATI add constraint FKR
     foreign key (id_servizio)
     references SERVIZI (id_servizio);

alter table SOTTOSCRIZIONI_ABBONAMENTO add constraint FKR_1
     foreign key (id_listino_abbonamento)
     references LISTINO_ABBONAMENTI (id_listino_abbonamento);

alter table SOTTOSCRIZIONI_ABBONAMENTO add constraint FKR
     foreign key (id_city_card)
     references CITY_CARD (id_city_card);

alter table SOTTOSCRIZIONI_ABBONAMENTO add constraint FKR_2
     foreign key (num_carta_credito)
     references CARTE_CREDITO (num_carta_credito);

alter table USERS add constraint FKR
